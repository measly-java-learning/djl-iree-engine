...
// shape longer than INT32_MAX dims cannot be a jlongArray. This header stays free of
// <jni.h> so the Catch2 units can pin the boundary without a JNIEnv, which is also why
// the limit is spelled INT32_MAX rather than the jsize type itself.
//
...
// iree_djl_jni.cpp) -- a heap write overflow, not a bounds-checked error. The
...
// True when n is too large to cast to jsize (jint) without wrapping/overflowing.
